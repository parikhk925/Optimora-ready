 ...
 * connection via connectGoogleCalendarIntegration. Redirects back to the
...
export async function GET(req: NextRequest) {
  ...
  if (!clientId || !clientSecret || !redirectUri) {
    return redirectToIntegrations(req, "error", "google_calendar_not_configured");
  }
  ...
  const result = await connectGoogleCalendarIntegration(ctx, {
